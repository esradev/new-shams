...
export class DownloadManager {
   ...
   */
  public addProgressListener(id: string, listener: (progress: DownloadProgress) => void): void {
  ...
  private async saveDownloadMetadata(file: DownloadedFile): Promise<void> {
    try {
      ...
      await AsyncStorage.setItem(STORAGE_KEYS.DOWNLOADED_FILES, JSON.stringify(updated));
    } catch (error) {
    ...
    if (file.localPath) {
      try {
        ...
        if (fileInfo.exists) {
          ...
          console.log(`Local file ${file.localPath} no longer exists, falling back to streaming`);
          return file.url;
        }
      } catch (error) {
  ...
  public async migrateOldDownloads(): Promise<void> {
    try {
      ...
      if (oldData) {
        ...
        await AsyncStorage.setItem(STORAGE_KEYS.DOWNLOADED_FILES, JSON.stringify(newDownloads));
        await AsyncStorage.removeItem('downloadedLessons');

        console.log(`Migrated ${newDownloads.length} downloads to new format`);
      }
    } catch (error) {
  ...
  public static showDownloadError(error: string): void {
    ...
    if (error.includes('Network')) {
      ...
      message = 'دسترسی به سیستم فایل امکان‌پذیر نیست. فایل به صورت آنلاین پخش خواهد شد.';
    }
