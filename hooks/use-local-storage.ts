...
export const useLocalStorage = () => {
  ...
  const loadAllData = () => {
    try {
      ...
      const completed = getStoredData<CompletedLesson[]>(
        STORAGE_KEYS.COMPLETED_LESSONS,
        []
      )
      const favorites = getStoredData<FavoriteLesson[]>(
        STORAGE_KEYS.FAVORITE_LESSONS,
        []
      )
      const downloaded = getStoredData<DownloadedLesson[]>(
        STORAGE_KEYS.DOWNLOADED_LESSONS,
        []
      )
      const activities = getStoredData<UserActivity[]>(
  ...
  const getStoredData = <T>(key: string, defaultValue: T): T => {
    try {
      const jsonValue = userStorage.getString(key)
  ...
  const markLessonCompleted = async (
    lesson: Omit<CompletedLesson, "completedAt">
  ) => {
  ...
  const markLessonIncomplete = (lessonId: string) => {
    ...
    setCompletedLessons(updatedCompleted)
    storeData(STORAGE_KEYS.COMPLETED_LESSONS, updatedCompleted)
    return true
  }
  ...
  const addToFavorites = (lesson: Omit<FavoriteLesson, "favoritedAt">) => {
    ...
    storeData(STORAGE_KEYS.FAVORITE_LESSONS, updatedFavorites)
  ...
  const removeFromFavorites = (lessonId: string) => {
  ...
  const addToDownloads = (lesson: Omit<DownloadedLesson, "downloadedAt">) => {
    ...
    storeData(STORAGE_KEYS.DOWNLOADED_LESSONS, updatedDownloads)
  ...
  const removeFromDownloads = (lessonId: string) => {
  ...
  const clearAllData = () => {
    try {
      userStorage.remove(STORAGE_KEYS.COMPLETED_LESSONS)
      userStorage.remove(STORAGE_KEYS.FAVORITE_LESSONS)
      userStorage.remove(STORAGE_KEYS.DOWNLOADED_LESSONS)
      userStorage.remove(STORAGE_KEYS.USER_ACTIVITIES)
